...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
            ...
            // ========================== 
            // -- Power Graphs v1.8.15 -- 
            // By Remaarn 
            // ========================== 
            // Updates: 
            // v1.8.15 - Added resetData command to reset saved information. 
            // v1.8.14 - Added margin setting. 
            // v1.8.13 - Fix for loading issue after enabling long history. 
            // v1.8.12 - Added BatteryInOutAuto graph, just like BatteryInOut except it is automatically scaled based on peak value. 
            // v1.8.11 - Improved display of missing timespans. Added Color_Brightness config option and reduced default brightness to 0.5. 
            // v1.8.10 - Improved graph type parsing. 
            // v1.8.9  - Added the usageScaled graph. See instructions for more details. 
            // v1.8.8  - Changed the format of graph type display specification to better cooperate with some other scripts. 
            // v1.8.7  - Changed the batteryInOut graph to use negative values for output. 
            // v1.8.6  - Added INI based config to the CustomData of program block. 
            // v1.8.5  - Separated display of peak and max values. 
            // v1.8.4  - Another fix for "Script too complex" error. 
            // v1.8.3  - Added fix for "Script too complex" error during first run on very complex ships. 
            // v1.8.2  - Compatibility fix for SE 1.192. 
            // v1.8.1  - Moved battery recharge input into separate curve for usage graph. 
            // v1.8    - Updated to use new LCD drawing APIs from new SE update 
            //           1.190. Now has much smoother graphics. 
            ...
            // 
            // * Tracking of gas tanks must be enabled in the script settings 
            // 
            ...
            //      NOTICE  NOTICE 
            // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
            // Configuration has been moved to the Custom Data of the progam block. 
    ...
    "Power Graphs -|--"
            ...
            readonly List<DisplayTypesKey> displayTypeKeys = new List<DisplayTypesKey>();
            ...
            struct LCDStatus
            {
                ...
                public DisplayTypesKey[] DisplayTypes;
            }

            struct DisplayTypesKey
            {
                public DisplayTypes DisplayTypes;
                public int SurfaceIndex;

                public DisplayTypesKey(DisplayTypes displayType, int surfaceIndex)
                {
            ...
            readonly Func<LCDStatus, bool> findLCDStatusByLCDPred;
            readonly Func<LCDStatus, bool> FindLCDStatusByNamePred;
            readonly Func<IMySensorBlock, bool> SensorCollectorFunc;

            IMyTextSurfaceProvider closureLCDPanel;
            string closureLCDName;
            ...
            bool FindLCDStatusByLCD(LCDStatus status) => status.LCD == closureLCDPanel;

            int FindLCDStatusByLCD(List<LCDStatus> statuses, IMyTextSurfaceProvider surfaceProvider)
            {
                closureLCDPanel = surfaceProvider;
                return statuses.FindIndex(findLCDStatusByLCDPred);
            }

            bool FindLCDStatusByName(LCDStatus status) => ((IMyTerminalBlock)status.LCD).CustomName == closureLCDName;
        ...
        new GraphSettings(GraphType.Single, "Hydrogen Tanks", 1, "L", "Stored Gas: ", Stored_Hydrogen_Color)
            ...
            bool UpdateSensors()
            {
                ...
                foreach (var sensor in sensors)
                {
                    ...
                    foreach (var item in lcdNames)
                    {
                        ...
                        int statusIndex = lcdStatuses.FindIndex(FindLCDStatusByNamePred);
            ...
            void UpdatePanel(IMyTerminalBlock block, DateTime now)
            {
                ...
                int statusIndex = FindLCDStatusByLCD(lcdStatuses, surfaceProvider);

                displayTypeKeys.Clear();

                int surfacesMask;
                int minSurfaceIndex;
                int maxSurfaceIndex;
                ParseSurfaceDisplayTypes(block.CustomData, surfaceProvider.SurfaceCount, displayTypeKeys, out surfacesMask, out minSurfaceIndex, out maxSurfaceIndex);

                int trackedSurfaceCount = displayTypeKeys.Count;
                ...
                if (statusIndex == -1)
                {
                    ...
                    lcdStatuses.Add(status = new LCDStatus
                    {
                        ...
                        DisplayTypes = new DisplayTypesKey[trackedSurfaceCount]
                    });

                    status.DisplayTypes[0].DisplayTypes = DisplayTypes.Invalid;
                }
                else
                {
                    ...
                    if (status.DisplayTypes.Length != trackedSurfaceCount)
                    {
                        status.DisplayTypes = new DisplayTypesKey[trackedSurfaceCount];
                        status.DisplayTypes[0].DisplayTypes = DisplayTypes.Invalid;
                    }
                }
            ...
            {
                int surfaceStatusIndex = -1;

                for (int i = 0; i < status.SurfaceToUpdate + 1; i++)
                {
                    if ((surfacesMask & 1 << i) != 0)
                        surfaceStatusIndex++;
                }

                var statusDisplayType = status.DisplayTypes[surfaceStatusIndex].DisplayTypes;
                bool wasInvalid = statusDisplayType == DisplayTypes.Invalid;

                if (wasInvalid || (now - status.LastCycle).TotalSeconds > Auto_Cycle_Interval_In_Seconds)
                {
                    var displayTypes = displayTypeKeys[surfaceStatusIndex].DisplayTypes;
                    statusDisplayType = GetNextDisplayType(statusDisplayType, displayTypes);
                    status.DisplayTypes[surfaceStatusIndex].DisplayTypes = statusDisplayType;
                    status.LastCycle = now;
                }

                if (status.IsCleared || (wasInvalid && statusDisplayType != DisplayTypes.Invalid))
                    status.IsCleared = false;

                var surface = surfaceProvider.GetSurface(status.SurfaceToUpdate);

                if (statusDisplayType == DisplayTypes.Invalid)
                {
                    surface.ContentType = ContentType.TEXT_AND_IMAGE;
                    surface.Font = "Debug";
                    surface.FontSize = 1f;
                ...
                else
                {
                    ...
                    UpdateDisplay(surface, statusDisplayType, isWide, now);
                }
                ...
                {
                    for (int i = status.SurfaceToUpdate + 1; i < maxSurfaceIndex + 1; i++)
                    {
                        if ((surfacesMask & 1 << i) == 0)
                            continue;
            ...
            void ParseSurfaceDisplayTypes(string text, int surfaceCount, List<DisplayTypesKey> keys, out int surfacesMask, out int minSurfaceIndex, out int maxSurfaceIndex)
            {
                surfacesMask = 0;
                ...
                while (!textPtr.IsOutOfBounds())
                {
                    ...
                    AddDisplayKey(keys, displayType, num);
                    minSurfaceIndex = Math.Min(minSurfaceIndex, num);
                    maxSurfaceIndex = Math.Max(maxSurfaceIndex, num);
                    surfacesMask |= 1 << num;
                }

                if (keys.Count == 0)
                {
                    keys.Add(new DisplayTypesKey(DisplayTypes.Auto, 0));
                    minSurfaceIndex = 0;
                    maxSurfaceIndex = 0;
                    surfacesMask = 1;
                }
            ...
            static void AddDisplayKey(List<DisplayTypesKey> keys, DisplayTypes displayType, int index)
            {
                bool replaced = false;
                ...
                for (int i = 0; i < keys.Count; i++)
                {
                    ...
                    if (key.SurfaceIndex != index)
                        continue;

                    key.DisplayTypes |= displayType;
                    keys[i] = key;
                    replaced = true;
                }

                if (!replaced)
                    keys.Add(new DisplayTypesKey(displayType, index));
            }

            static DisplayTypes GetDisplayType(StringSegment typeString)
            {
                if (typeString.EqualsIgnoreCase(new StringSegment("usage"))) return DisplayTypes.PowerUsage;
                if (typeString.EqualsIgnoreCase(new StringSegment("usageScaled"))) return DisplayTypes.PowerUsageScaled;
                else if (typeString.EqualsIgnoreCase(new StringSegment("battery"))) return DisplayTypes.BatteryCharge;
                else if (typeString.EqualsIgnoreCase(new StringSegment("batteryIn"))) return DisplayTypes.BatteryIn;
                else if (typeString.EqualsIgnoreCase(new StringSegment("batteryOut"))) return DisplayTypes.BatteryOut;
                else if (typeString.EqualsIgnoreCase(new StringSegment("batteryInOut"))) return DisplayTypes.BatteryInOut;
                else if (typeString.EqualsIgnoreCase(new StringSegment("batteryInOutAuto"))) return DisplayTypes.BatteryInOutAuto;
                else if (typeString.EqualsIgnoreCase(new StringSegment("solar"))) return DisplayTypes.SolarOutput;
                else if (typeString.EqualsIgnoreCase(new StringSegment("reactor"))) return DisplayTypes.ReactorOutput;
                else if (typeString.EqualsIgnoreCase(new StringSegment("h2engine"))) return DisplayTypes.H2PowerGenOutput;
                else if (typeString.EqualsIgnoreCase(new StringSegment("wind"))) return DisplayTypes.TurbineOutput;
                else if (typeString.EqualsIgnoreCase(new StringSegment("storedOxygen"))) return DisplayTypes.StoredOxygen;
                else if (typeString.EqualsIgnoreCase(new StringSegment("storedHydrogen"))) return DisplayTypes.StoredHydrogen;
            ...
            void RecordValues(List<IMyTerminalBlock> blocks)
            {
                ...
                GetPowerState(blocks, out currentPowerUsage, out currentPowerRequired, out maxPowerRequired);
                ...
                GetBatteryValues(blocks, out storedPower, out maxStoredPower,
                ...
                float reactorPower;
                GetReactorOutput(blocks, out reactorPower, out reactorOutputMaxValue);
                AddValueToArray(reactorOutputValues, reactorPower);

                float generatorPower;
                GetGeneratorOutput(blocks, out generatorPower, out h2GenMaxOutputValue);
                AddValueToArray(h2PowerGenOutputValues, generatorPower);

                float windMaxPower, windPower;
                GetTurbineOutput(blocks, out windMaxPower, out windPower);
                ...
                if (Measure_Stored_Gas)
                {
                    ...
                    GetStoredGas(blocks, out oxygenCapacity, out storedOxygen, out hydrogenCapacity, out storedHydrogen);
            ...
            void GetPowerState(List<IMyTerminalBlock> blocks, out float currentPowerUsage, out float currentPowerRequired, out float maxPowerRequired)
            {
                currentPowerUsage = 0;
            ...
            void GetBatteryValues(List<IMyTerminalBlock> blocks,
                ...
                out float output, out float maxOutput)
            {
                ...
                blocks.GetItemsOfType(batteries, BlockCollectorFunc);
            ...
            void GetReactorOutput(List<IMyTerminalBlock> blocks, out float reactorPower, out float maxOutput)
            {
                reactorPower = 0;
                maxOutput = 0;
            ...
            void GetGeneratorOutput(List<IMyTerminalBlock> blocks, out float generatorPower, out float maxOutput)
            {
                generatorPower = 0;
                maxOutput = 0;
                ...
                for (int i = 0; i < blocks.Count; i++)
                {
                    ...
                    if (!(item is IMyPowerProducer) || item.GetType().Name != "MyHydrogenEngine")
                        continue;

                    var generator = (IMyPowerProducer)item;
                    h2PowerGens.Add(generator);
                    generatorPower += generator.CurrentOutput;
                    maxOutput += generator.MaxOutput;
                }
            }

            void GetTurbineOutput(List<IMyTerminalBlock> blocks, out float windMaxPower, out float windPower)
            {
                windMaxPower = 0;
                windPower = 0;
                ...
                for (int i = 0; i < blocks.Count; i++)
                {
                    ...
                    if (item is IMyPowerProducer && item.GetType().Name == "MyWindTurbine")
                    {
                        var turbine = (IMyPowerProducer)item;

                        turbines.Add(turbine);
                        windMaxPower += turbine.MaxOutput;
                        windPower += turbine.CurrentOutput;
                    }
                }
            }

            void GetStoredGas(List<IMyTerminalBlock> blocks, out float oxygenCapacity, out float storedOxygen, out float hydrogenCapacity, out float storedHydrogen)
            {
                oxygenCapacity = 0;
                storedOxygen = 0;
                hydrogenCapacity = 0;
                ...
                oxygenTankCount = 0;
                hydrogenTankCount = 0;
                blocks.GetItemsOfType(gasTanks);
